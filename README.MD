...
## Custom Album Art
* In the `clientdata` folder you will find 5 text files by default. `chrome.dat` `groove.dat` and `spotify.dat` are required only to provide a default if there is no provided album art.
